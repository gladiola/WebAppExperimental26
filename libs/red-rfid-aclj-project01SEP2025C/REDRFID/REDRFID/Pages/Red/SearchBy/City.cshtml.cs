...
namespace REDRFID.Pages.Red.SearchBy
{
    public class CityModel : PageModel
    {
        ...
        public CityModel(RedCosmosDBContext context, ILogger<CityModel> logger, ICosmosDbSettingsService cosmosDbSettingsService)
        {
            _context = context;
            _logger = logger;
        ...
        {
            string caller = "/Red/SearchBy/CityModel.OnGetAsync()";
        ...
        {
            string caller = "/Red/DataEntry/CityModel.OnPostAsync()";
            ...
            string idOut = Convert.ToString(City);
            LoggingHelper.LogDataProcessingStatusServiceWork(_logger, caller, DataProcessingStatus.Info, $"Search term to look for (City):\t{idOut}");
            ...
            try
            {
                using (_context)
                {
                    ...
                    var query = new QueryDefinition(query: "SELECT * FROM c WHERE c.LocationCity = @City").WithParameter("@City", idOut);
                    ...
                    if (RedIdRecord == null)
                    {
                        LoggingHelper.LogDataProcessingStatusServiceWork(_logger, caller, DataProcessingStatus.Info, $"Item to search for was not found.  City: {idOut}");
                    }
